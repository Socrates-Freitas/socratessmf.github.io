...
function drawClock(place) {
  ...
  drawClock.decimals.map((n, i) => {
    ...
    var coord = polar2Cartesian(1.01 * clockRadius, i * fiveMin * 0.5 * -1); // inverter numeros externos
    // translate to the center of the canvas
    coord = translate(coord, center);
    context.fillText(n.txt, coord.x, coord.y);
  });
...
var runAnimation = (() => { // closure que executa automaticamente
  ...
  function drawHandles() {
    ...
    clock_handles[0].time2Angle = fiveMin * (+hours12 + minutes / 60) * -1;
    clock_handles[1].time2Angle = oneMin * (+minutes + seconds / 60) * -1;
    clock_handles[2].time2Angle = oneMin * seconds * -1;
    clock_handles[3].time2Angle = fiveMin * (+hours + minutes / 60) * 0.5 * -1;
